...
module.exports.runDocker = (config, backstopCommand) => {
  if (config.args.docker) {
    ...
    const DOCKER_COMMAND = `docker run --rm -i --mount type=bind,source="${process.cwd()}",target=/src backstopjs/backstopjs:${version} ${backstopCommand}${configArgs} "${passAlongArgs}"`;
